...
class Accuracy(BaseMetric):
    ...
    def process(self, data_batch, data_samples):
        ...
        prediction_lens = torch.tensor([torch.count_nonzero(pred != tokenizer.pad_token_id) for pred in outputs], dtype=torch.float64)
    ...
    def compute_metrics(self, results):
        return dict(gen_len=np.mean([item['gen_len'] for item in results]),
